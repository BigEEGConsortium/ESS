...
elseif ~iscell(value) && ~isequal(value,[]) && ~isempty(default_val)
    error(['For an arg_subtoggle argument that has non-empty defaults (here:' spec.names{1} '), it is not allowed to assign anything other than a cell array, a struct, or [] to it. You are assigning: ' hlp_tostring(value)]);
end
