function [datac, f0Mask] = removeLinesMovingWindow(data, f0, noisyParameters)
% Removes significant sine waves from (continuous) data using overlapping windows.
%
% Usage: 
% [datac, datafit, Amps, freqs, significant] =  ...
%                                          removeLinesMovingWindow(data, g)[datac,datafit]=rmlinesmovingwinc(data,movingwin,tau,params,p,plt)
%
%  Inputs:  
...
[~, tau] = getStructureParameters(noisyParameters, 'tau');

% Window,overlap and frequency information
data = change_row_to_column(data);
N = size(data, 1);
Nwin = round(Fs*noisyParameters.taperWindowSize); % number of samples in window
Nstep = round(noisyParameters.taperWindowStep*Fs); % number of samples to step through
...
for n = 1:nw
   ...
   datawin = data(indx);
   [datafitwin, as, fs, Fval, sig, f0Sig] = ...
       fitSignificantFrequencies(datawin, f0, noisyParameters);
   Amps{n} = as;
   freqs{n} = fs;
