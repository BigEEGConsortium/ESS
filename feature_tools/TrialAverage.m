classdef TrialAverage < Block
    ...
    methods
        function obj = TrialAverage(trialBlock, varargin)
            obj = obj@Block;
            obj = obj.defineAsSubType('TrialAverage');
            obj = obj.setId;
            
            if nargin > 0
                if isa(trialBlock, 'Block')
                   
                    
                    
                    obj.axes{1} = TrialGroupAxis('groups', {trialBlock.trial});
                    obj.axes{2} = FeatureAxis('names', {'mean', 'standard deviation', 'standard deviation of mean', 'median', 'median absolute deviation' 'mean zscored'});
                    ...
                    obj.tensor(1, 6,:) = vec(mean(zscore(trialBlock.index('trial', nonTrialAxisLabels{:}), 0, 2)));
                    
                    obj.featureType = trialBlock.type;
                else
                    error('The first input value should be of type ''Block''');
                end;
            end;
        end
