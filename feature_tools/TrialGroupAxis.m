classdef TrialGroupAxis < InstanceAxis
    ...
    properties
        commonHedStrings % HED strings that all the trials in the group match to. Mandatory if trial groups were selected by matching to a single HED string.
        commonEventCodes % Event codes common to all the trials in the group match to. Mandatory if trial groups were selected by event codes. 
                         % A combination of 'commonHedStrings' and 'commonHedStrings' with AND
                         % operator is assumed when both are provided for a group.
    ...
    methods
        function obj =  TrialGroupAxis(varargin)
            ...
            obj.perElementProperties = [obj.perElementProperties {'groups' 'commonHedStrings' 'commonEventCodes' 'descriptions'}];
            ...
            inputOptions = arg_define(varargin, ...
                ...
                arg('commonEventCodes', {}, {},'A cell array with trial groups event codes. All trials in each group match the corresponding ''common event code''.', 'type', 'cellstr'),...                
            ...
            if length(length(inputOptions.groups)) < max(length(length(inputOptions.commonHedStrings)), length(length(inputOptions.commonEventCodes)),length(inputOptions.descriptions))
                error('Missing items in ''groups'' variable (at least one other per-item array contains more items)');
            end;
            
            otherfields = {'commonHedStrings' 'commonEventCodes' 'descriptions' 'payloads'};
            ...
            if isempty(inputOptions.commonEventCodes)
                for i=1:length(inputOptions.groups)
                    inputOptions.commonEventCodes{i} = '';
                end;
            end;
            ...
            obj.commonEventCodes = inputOptions.commonEventCodes(:);           
